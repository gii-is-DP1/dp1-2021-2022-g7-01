...
public class ParadaTest {
	...
	void paradaCheck() {
		...
		List<Player> opponents= g.getListPlayers();
		opponents.remove(POVplayer);
		Collections.shuffle(opponents);
		String playerA= opponents.get(0).getUser().getUsername();
		...
		assertThat(handBefore==(p.getHand().size()+1));
